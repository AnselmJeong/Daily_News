# Weekly News — 뉴로사이언스 논문 주간 브리프

`Articles/` 폴더에 새로 추가되는 PDF들을 모아 주간 HTML 다이제스트로
만들어 주는 파이프라인. 단순한 요약을 넘어, **카테고리별로 영속되는 "Living
Cluster"** 레이어를 유지해서 한 번 형성된 주제가 어떻게 확장·개명·휴면·부활·병합
되는지를 시간에 따라 추적한다.

- 실행 명령: `weekly-news`
- 주간 산출: `_news/daily-YYYY-MM-DD.html`
...
| 단계 | 내용 |
|------|------|
| **Harvest** | `index.json`의 주간 window 엔트리. `_uncategorized`와 root-level 미분류 PDF는 weekly issue 본문에서 제외. |
...
- **주 1회**: `weekly-news` — 위 전체 파이프라인.
- **주 1회(권장)**: `weekly-news --consolidate` — 전역 housekeeping.
...
├── <Category 2>/
│   └── *.pdf
├── _uncategorized/                    # 아직 분류 안 된 PDF들; weekly issue 본문에서는 제외
└── _news/                             # 산출물 + 캐시
    ├── index.html                     # 롤업 페이지
...
성공하면 PATH에 `weekly-news` 커맨드가 노출된다. 기존 자동화 호환을 위해 `daily-news` alias도 유지된다.

```bash
which weekly-news     # ~/.local/bin/weekly-news
weekly-news --help
```
...
```bash
weekly-news --articles-root /path/to/Articles
ARTICLES_ROOT=/path/to/Articles weekly-news
```
...
0 3 * * 0 cd /Volumes/Aquatope/_DEV_/Daily_News/articles && /opt/homebrew/bin/uv run weekly-news --consolidate >> _news/.cache/consolidate.log 2>&1
```
...
weekly-news --consolidate --dry-run --verbose

# 괜찮으면 실제로 적용
weekly-news --consolidate
```
...
- **Top Themes** — Hero 우측에서 주간 논문 수와 최근 baseline 논문 수를 합산해 가장 큰 주제 4개를 표시.
- **Cluster Evolution** ← 새 섹션. 🌿 Born / ➕ Extended / 🔀 Renamed / ✨ Revived
  아이템. 각 항목 클릭 시 해당 LC 상세 페이지로 이동.
- **This Week's Reading** — 카테고리별 paper 카드. cluster-head 우상단에
  `NEW` / `+N this week` / `RENAMED` / `REVIVED` 뱃지, 그리고 **10주 sparkline**이
...
# 전부 새로 태우기 (처음부터 재구축)
rm -rf _news/.cache/
weekly-news --date 2026-04-19     # 혹은 여러 날짜 순회
```
...
# consolidate dry-run
uv run weekly-news --consolidate --dry-run --verbose

# 특정 날짜 rebuild
uv run weekly-news --date 2026-04-19 --verbose

# 2026-04-01부터 2026-04-19까지 추가분을 한 번에 처리
uv run weekly-news --from 2026-04-01 --to 2026-04-19 --verbose
```
